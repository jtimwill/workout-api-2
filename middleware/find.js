...
async function findWorkout(req, res, next) {
  const workout = await Workout.findOne({ where: { id: req.params.workoutId }}); // changed from body
  if (!workout) {
...
async function findCompletedWorkout(req, res, next) {
  const completed_workout = await CompletedWorkout.findOne({ where: { id: req.params.completedWorkoutId }}); // changed from body
